...
def run_train_hybrid_qcnn(config):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset)):
        ...
        # -- Création explicite du pool --
        pool = None
        if config.get("training", {}).get("parallel", True):
            pool = mp.Pool(mp.cpu_count())
            print(f"[Fold {fold}] Pool multiprocessing créé avec {mp.cpu_count()} processus.")
        ...
        try:
            ...
            for epoch in trange(start_epoch, EPOCHS, desc=f"[Fold {fold}] Hybrid QCNN Training"):
                ...
                for batch_X, batch_y in tqdm(train_loader, desc=f"[Fold {fold}] Batches"):
                    ...
                    optimizer.zero_grad()
                    outputs = model(batch_X, pool=pool).squeeze()  # <-- Passe pool explicitement
                ...
                with torch.no_grad():
                    for batch_X, batch_y in val_loader:
                        ...
                        preds_logits = model(batch_X, pool=pool).squeeze()  # <-- Passe pool explicitement
            ...
            if pool is not None:
                print(f"[Fold {fold}] Fermeture explicite du pool multiprocessing.")
                pool.close()
                pool.join()
                pool = None
