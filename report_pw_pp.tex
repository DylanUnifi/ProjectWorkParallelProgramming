...
\begin{itemize}
    ...
    \item \textbf{GPU custom}: generate states with PyTorch, convert tensors to CuPy via DLPack, 
    and compute the Gram with a custom \textit{CUDA kernel}.
\end{itemize}
...
\caption{Unified backend API: the Gram matrix is computed on GPU (\textbf{GPU base} or \textbf{GPU custom}) or CPU (\textbf{CPU base}); the SVM solver always runs on CPU.}
...
\label{fig:cuda-flow}
\end{subfigure}
\caption{Horizontal comparison of backend pipelines referenced in the HPC architecture section.}
\end{figure*}
...
\caption{Opt knobs.}
\end{subfigure}
\caption{Kernel construction in \textbf{GPU custom}, organized across three full-width elements: host orchestration, device kernel core, and optimization controls.}
\label{fig:rawkernel}
\end{figure*}

\subsubsection{Backend Optimizations}
Backend-specific optimizations are grouped to clarify what changes execution speed and what preserves metric comparability:
\begin{itemize}
    \item \textbf{CPU base path}: CPU-only reference path with no VRAM-aware tiling, stream orchestration, or kernel autotuning.
...
The observed best settings were vram\_fraction=0.95 and num\_streams=1,
with pinned memory + streams on the \textbf{GPU base} path.

\subsubsection{Normalization and Metric Comparability}
All backend comparisons share the same post-processing and kernel normalization protocol so that F1/AUC and runtime differences can be interpreted as backend effects rather than normalization artifacts:
\begin{itemize}
...
\begin{table}[t]
\centering
\caption{Sample-scaling throughput (Mpairs/s) by dataset and backend from per-dataset benchmark CSVs. Values are reported at the highest sample-scaling qubit setting available in each dataset profile (Fashion: 12 qubits; CIFAR-10 and SVHN: 16 qubits).}
\label{tab:benchmark}
\scriptsize
...
The benchmark plots in Figs.~\ref{fig:bench-fashion}, \ref{fig:bench-cifar10}, and \ref{fig:bench-svhn} are consistent with Table~\ref{tab:benchmark}: backend ranking depends on both dataset profile and sample size.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{benchmark_results/fashion/benchmark.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{benchmark_results/cifar10/benchmark.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{benchmark_results/svhn/benchmark.png}
...
Table~\ref{tab:classification-summary} shows that, within quantum backends, \textbf{GPU base} is generally faster and often more accurate than \textbf{GPU custom}, while both remain strong on Fashion and weak on SVHN.
...
\begin{table}[t]
\centering
\caption{Run-scoped paired comparison by sample size. Positive deltas indicate better classical quality; negative time deltas indicate faster classical execution.}
...
These observations indicate that acceleration alone does not remove
the representational limits of the feature map.
...
\section{Conclusion}
The updated benchmark and classification tables show that backend ranking is not global: it depends on sample size, dataset profile, and backend implementation details. 
In the benchmark campaign, GPU paths dominate low-size regimes,
while some larger-size points still favor \textbf{CPU base} in specific profiles.

On predictive quality, both quantum backends remain competitive on Fashion-MNIST but degrade on CIFAR-10 and especially SVHN, where collapse indicators and near-random AUC values appear repeatedly in hard settings.
Overall, acceleration improves throughput but does not remove the representational bottleneck of the current feature map/ansatz pair on harder visual distributions.
